...
def get_story_comments(story_Id):
  ...
  if story:
    base = [comment.to_dict(int(current_user.is_authenticated) and current_user.id) for comment in story.comments if comment.parent_id is None] # Can be improved
    return jsonify(base)
  else:
...
def get_comment_replies(comment_Id):
  ...
  if comment:
    res = comment.to_dict(int(current_user.is_authenticated) and current_user.id)
    res['replies'] = {reply.id: reply.to_dict(int(current_user.is_authenticated) and current_user.id) for reply in comment.replies}
    return jsonify(res)
  else:
...
def reply_to_a_comment(comment_id):
  ...
  if comment:
    if form.validate_on_submit:
      ...
      db.session.commit()
      return reply.to_dict(int(current_user.is_authenticated) and current_user.id)
    else:
      return {'errors': validation_errors_to_error_messages(form.errors)}, 401
  else:
    return jsonify({'message': 'Comment could not be found'}), 404
